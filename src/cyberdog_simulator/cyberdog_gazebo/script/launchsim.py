...
def launchsim(extra_args):
    ...
    os.system(
        f'NO_AT_BRIDGE=1 gnome-terminal --title="cyberdog_gazebo" -- bash -c "{gazebo_cmd}"'
    )
    time.sleep(5)
    os.system(
        f'NO_AT_BRIDGE=1 gnome-terminal --title="cyberdog_viusal" -- bash -c "{visual_cmd}"'
    )
    os.system(
        'NO_AT_BRIDGE=1 gnome-terminal --title="cyberdog_control" -- bash -c "bash ./src/cyberdog_simulator/cyberdog_gazebo/script/launchcontrol.sh"'
    )

...
template < typename T >
bool CheckBodyHeightLegSafety( const StateEstimatorResult< T >& se_result, Quadruped< T >& quadruped, LegController< T >& leg_controller, const T& body_height_des,
                               const Vec3< T >& body_rpy_des ) {
    Vec3< T > pos_foot[ 4 ];
    for ( int i = 0; i < 4; ++i ) {
    ...
    Eigen::Matrix< T, 3, 3 > body_rot_des = RpyToRotMat( body_rpy_des );
    Vec3< T >                body_pos_des( se_result.position[ 0 ], se_result.position[ 1 ], body_height_des );

    for ( int i = 0; i < 4; ++i ) {
        Vec3< T > hip_pos_des = body_pos_des + body_rot_des.transpose() * quadruped.GetHipLocation( i );
        Vec3< T > leg_pos_des = body_rot_des * ( hip_pos_des - pos_foot[ i ] );
        T         leg_length  = leg_pos_des.norm();

        if ( leg_length > quadruped.max_leg_length_ || leg_length < static_cast< T >( 0.35 ) * quadruped.max_leg_length_ || std::fabs( leg_pos_des[ 2 ] ) < static_cast< T >( 0.11 ) ) {
            return false;
        }
    }

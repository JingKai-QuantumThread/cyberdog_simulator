...
#else
        body_height_lim_des_ = ApplyPoseMeetVelocityAccelationLimits( ( double )body_height_last_, ( double )body_height_vel_cur_, body_height_cmd, user_param->qp_stand_height_min,
                                                                      user_param->qp_stand_height_max, -0.5, 0.5, -3., 3., dt );
#endif
...
template < typename T > bool FsmStateBalanceStand< T >::LegSafetyChecker() {
    ...
    return CheckBodyHeightLegSafety( se_result, quadruped, leg_controller, body_height_lim_des_, rpy_des );
}

...
template < typename T > void FsmStatePoseCtrl< T >::Run() {
    ...
    if ( cmd->gait_id != 5 ) {  // pose increase
        ...
        if ( transFirshRun_ ) {
            ...
            if ( height_ < ( T )user_param->pose_transform_height_min )
                height_ = ( T )user_param->pose_transform_height_min;
            else if ( height_ > ( T )user_param->pose_transform_height_max )
                height_ = ( T )user_param->pose_transform_height_max;
            T com_x  = 0.0;

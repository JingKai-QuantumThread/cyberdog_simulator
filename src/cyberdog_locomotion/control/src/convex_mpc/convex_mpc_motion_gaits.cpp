...
void ConvexMpcMotionGaits::Initialize( ControlFsmData< float >& data ) {
    ...
    stair_height_vel_cur_   = 0.0;
    stair_last_safe_height_ = state_est_.position( 2 );
    stair_height_unsafe_    = false;
...
void ConvexMpcMotionGaits::SetCurrentGait() {
    ...
    if ( current_gait_ != gait_cmd_ ) {
        ResetIter();
        if ( IsStairMotionGait( gait_cmd_ ) ) {
            stair_height_vel_cur_   = 0.0f;
            stair_last_safe_height_ = state_est_.position( 2 );
            stair_height_unsafe_    = false;
        }
    }
...
void ConvexMpcMotionGaits::SetGaitParams( const int gait_id ) {
    ...
    case GaitId::kPitchDownRight:
        ...
        break;
    case GaitId::kStairUp:
    case GaitId::kStairDown:
    case GaitId::kSlope:
    case GaitId::kTofStair:
        SetStairMotionParams( gait_id );
        break;
...
void ConvexMpcMotionGaits::SetStairMotionParams( const int gait_id ) {
    ( void )gait_id;

    // Stair gaits reuse a conservative gait template and accept absolute body height.
    gait_type_ = &trot_in_out_;

    for ( int i = 0; i < 3; i++ ) {
        vel_cmd_min_( i ) = user_params_->vel_xy_yaw_min_stair[ i ];
        vel_cmd_max_( i ) = user_params_->vel_xy_yaw_max_stair[ i ];
    }

    rpy_cmd_[ 1 ]       = ctrl_cmd_->rpy_des[ 1 ];
    body_pitch_filter_  = 0.1;
    pos_cmd_[ 2 ]       = ctrl_cmd_->pos_des[ 2 ];
    body_height_filter_ = 0.01;
...
void ConvexMpcMotionGaits::SetCmd( ControlFsmData< float >& data ) {
    ...
    // set pos_des_
    if ( IsStairMotionGait( current_gait_ ) ) {
        ApplyStairHeightSafety();
    }
    else {
...
bool ConvexMpcMotionGaits::IsStairMotionGait( int gait_id ) const {
    return gait_id == GaitId::kStairUp || gait_id == GaitId::kStairDown || gait_id == GaitId::kSlope || gait_id == GaitId::kTofStair;
}

void ConvexMpcMotionGaits::ApplyStairHeightSafety() {
    if ( pos_cmd_[ 2 ] <= 0.0f ) {
        if ( !stair_height_unsafe_ ) {
            std::cout << "[MOTION GAIT] stair gait body height command invalid (" << pos_cmd_[ 2 ] << " m), hold last safe height " << stair_last_safe_height_ << " m"
                      << std::endl;
        }
        pos_des_( 2 )          = stair_last_safe_height_;
        stair_height_unsafe_   = true;
        stair_height_vel_cur_  = ( pos_des_( 2 ) - pos_des_last_[ 2 ] ) / dt_;
        return;
    }

    const float height_min     = ( float )user_params_->user_gait_height_min;
    const float height_max     = ( float )user_params_->user_gait_height_max;
    const float height_vel_max = ( float )user_params_->user_gait_height_vel_max;
    const float height_acc_max = ( float )user_params_->user_gait_height_acc_max;
    const float target_height  = WrapRange( pos_cmd_[ 2 ], height_min, height_max );

    float limited_height = ApplyPoseMeetVelocityAccelationLimits( pos_des_last_[ 2 ], stair_height_vel_cur_, target_height, height_min, height_max, -height_vel_max, height_vel_max, -height_acc_max,
                                                                  height_acc_max, dt_ );

    if ( CheckBodyHeightLegSafety( state_est_, *robot_model_, *leg_ctrl_, limited_height, rpy_des_ ) ) {
        if ( stair_height_unsafe_ ) {
            std::cout << "[MOTION GAIT] stair gait body height safety recovered at " << limited_height << " m" << std::endl;
        }
        pos_des_( 2 )            = limited_height;
        stair_last_safe_height_  = limited_height;
        stair_height_unsafe_     = false;
    }
    else {
        if ( !stair_height_unsafe_ ) {
            std::cout << "[MOTION GAIT] stair gait body height command rejected by leg safety, hold last safe height " << stair_last_safe_height_ << " m" << std::endl;
        }
        pos_des_( 2 )          = stair_last_safe_height_;
        stair_height_unsafe_   = true;
    }
